...
app.get('/todos', (req, res) => {
  const { page = 1, limit = 10, filter = 'all', sort = 'newest' } = req.query;
  let todos = readTodos();
...
app.put('/todos/:id', (req, res) => {
  ...
  if (text !== undefined) {
    ...
    const isExist = todos.some(
      (todo) =>
        todo.id !== todoId &&
        todo.text.toLowerCase() === normalizedText.toLowerCase(),
    );
    ...
    todos[todoIndex].text = normalizedText;
  }
